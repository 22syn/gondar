...
// Transport lives in d1Client on this branch (extracted during slice 1) rather
// than inside setupD1Ingest as on `main` — three ingests share it now.
...
export async function ingestFragilityToD1(
    ...
    try {
        ...
        // Uses the shared d1ConfigFromEnv rather than rebuilding the env lookup
        // inline as on `main` — same semantics (all three vars required, else
        // skip), one definition across the three ingests. The explicit `cfg`
        // parameter is preserved for scripts/ingest-fragility.ts.
